...
{
    int blur_size = width * height;
    if (in == NULL && rank != 0)
    {
        in = (unsigned char *)calloc(blur_size, sizeof(unsigned char));
    }

    MPI_Bcast(in, blur_size, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
...
void mpi_edge_detect_halo(unsigned char *in, unsigned char *out, const int height, const int width, const int size, const int rank)
{
    ...
    // Note: If you are not Rank 0, your 'actual' data starts at row 1 of local_in
    int start_y = (rank == 0) ? 1 : 1;
    int local_height = send_counts[rank] / width;
    int start_y = (rank == 0) ? 0 : 1;
